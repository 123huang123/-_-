...
for user in data1["userId"].unique():
    ...
    user_all_time_max = (data1[data1["userId"] == user]["timestamp"].max()) / 604800
    user_nochange_time_max = user_all_time_max + 60

    user_all_time_min = (data1[data1["userId"] == user]["timestamp"].min()) / 604800
    user_nochange_time_min = user_all_time_min - 60
...
#找出全部电影邻居，进行计算预估评分
    ...
    for last_movie in last_recommend_movies:
        last_movie_all_time_max=(data1[data1["movieId"]==last_movie]["timestamp"].max())/1296000

        last_movie_all_time_min = (data1[data1["movieId"] == last_movie]["timestamp"].min())/1296000
