...
for user in data1["userId"].unique():
    ...
    user_all_time_max = (data1[data1["userId"] == user]["timestamp"].max()) / 2592000
    user_nochange_time_max = user_all_time_max + 30

    user_all_time_min = (data1[data1["userId"] == user]["timestamp"].min()) / 2592000
    user_nochange_time_min = user_all_time_min - 30
...
#找出全部电影邻居，进行计算预估评分
    ...
    for last_movie in last_recommend_movies:
        last_movie_all_time_max=(data1[data1["movieId"]==last_movie]["timestamp"].max())/2592000

        last_movie_all_time_min = (data1[data1["movieId"] == last_movie]["timestamp"].min())/2592000

        if (user_nochange_time_max<last_movie_all_time_min) or (user_nochange_time_min>last_movie_all_time_max):
            continue
