...
user_based_sim_option = {'name': 'cosine', 'user_based': False}
reader = Reader()
...
m = raw2inner_id_items
f = {value: key for key, value in m.items()}
# 训练并测试算法,建立模型
w = algo.fit(dataset)
s = w.sim
get_neighbors = {}
for col in data["movieId"].unique() :
    dict2 = {}
    c=[]
    A2 = m[col]  # col用户对于评分缺失物品的内部id
    w1 = w.get_neighbors(A2,10)  # 此物品100个邻居
    for r in w1:
        sim_score=s[r][A2]
        if sim_score>0:
            c.append(f[r])
