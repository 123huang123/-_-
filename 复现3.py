...
for user in data1["userId"].unique():
    # if user in [540, 502, 329, 399, 544, 508, 392, 180, 507, 467, 574, 236, 530, 55, 26, 283, 272, 47, 5, 208, 134, 485, 295, 85, 302, 146, 568, 449, 355, 238, 255, 145, 335, 35, 547, 10, 41, 496, 189, 433, 207, 206, 60, 31, 372, 14, 497, 386, 34, 319, 107, 423, 336, 513, 48, 224, 265, 8, 154, 539, 390, 138, 183, 551, 308, 418, 87, 150, 358, 406, 598, 584, 435, 519, 536, 538, 323, 147, 333, 457, 516, 86, 548, 444, 164, 404, 188, 385, 161, 176, 324, 515, 422, 51, 450, 227, 70, 102, 270, 108, 445, 151, 431, 369, 32, 452, 315, 39, 209, 158, 97, 33, 400, 455, 394, 542, 500, 148, 456, 454, 167, 211, 56, 250, 488, 40, 96, 427, 284, 81, 564, 537, 260, 143, 170, 374, 476, 242, 451, 136, 99, 429, 114, 254, 354, 577, 205, 514, 203, 109, 281, 464, 304, 61, 609, 230, 286, 543, 578, 565, 42, 19, 2, 458, 524, 22, 378, 555, 339, 410, 271, 344, 595, 142, 28, 434, 432, 523, 202, 38, 382, 37, 50, 218, 93, 275, 525, 130, 65, 419, 101, 366, 368, 481, 511, 462, 1, 215, 340, 346, 259, 58, 166, 220, 195, 397, 15, 424, 94, 168, 133, 6, 337, 221, 193, 234, 381, 307, 11, 504, 428, 603, 585, 316, 520, 610, 124, 477, 178, 80, 111, 325, 471, 583, 570, 357, 453, 387, 559, 421, 438, 532, 244, 389, 104, 139, 172, 210, 575, 412, 49, 320, 36, 121, 498, 596, 463, 63, 562, 197, 590, 376, 68, 430, 155, 177, 550, 599, 420, 18, 587, 135, 480, 345, 558, 305, 482, 232, 199, 246, 380, 483, 553, 503, 165, 64, 256, 499, 288, 144, 132, 469, 448, 122, 222, 239, 249, 285, 601, 292]:
    #     continue


    data_user=data1[data1["userId"]==user]
    data_user_own_movies=list(data_user["movieId"])
    time_max=data_user["timestamp"].max()
    time_min = data_user["timestamp"].min()
...
#找出全部电影邻居，进行计算预估评分
    ...
    for last_movie in last_recommend_movies:
        ...
        for data3_movie in data3_movies:
            time_now=list(data_user[data_user["movieId"]==data3_movie]["timestamp"])[0]
            last_movie_sim_sum+=nei_movies_dict[data3_movie]

            last_movie_sum+=(nei_movies_dict[data3_movie]*list(data3[data3["movieId"] == data3_movie]["rating"])[0])*(1-math.exp((time_now-time_max)/(time_max-time_min)))
